...
def get_results(addr,interface_name):
    ...
    wks.update_value("H4",interface_name + ':')
    wks.update_value("H4",reponse.text)
    print (parameter)
    return reponse 
...
#获取价格
reponse=get_results('C4','/order/price')
orderSN=reponse.json()['data']['orderSN']

#create order
reponse=get_results('D4','/order/add')
order_status=reponse.json()['data']['status']
assert order_status == 1 

##审核通过
reponse=get_results('E4','/order/audit')
...
# ##订单完成，不退款
# sql= "update popularize_status_new set order_id = \"%s\",expose_count ='60001',follow_count='0'   where update_time = ('1575826750') " % orderSN
# mysql_update(sql)
# reponse=get_results('F4','/order/audit')

# ##查询订单
# reponse=get_results('F4','/order/get')
# orderSN4=reponse.json()['data']['orderInfos'][0]
# #print(type(orderSN4))

# orderinfo=eval(str(wks.get_value("I4")))
# #比较结果
